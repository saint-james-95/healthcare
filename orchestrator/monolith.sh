...
while : ; do
	...
	echo "waiting for preprocessing to finish, current command is ${STATUS}. You can check the containers activities with sudo docker logs --follow hadoop1-master"
	sleep 300
done

sudo docker exec -ti hadoop1-master sh -c "cd /big/medicare-demo/ref_data && hadoop fs -get medicare_\$PCT"
#sudo docker stop hadoop1-master
#sudo docker rm hadoop1-master
